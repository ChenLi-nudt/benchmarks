...
__global__ void
ComputePhiMag_GPU222(float* phiR, float* phiI, float* phiMag, int numK) {
...
__global__ void
ComputeQ_GPU222(int numK, int kGlobalIndex,
...
void computePhiMag_GPU222(int numK, float* phiR_d, float* phiI_d, float* phiMag_d)
{
  ...
  ComputePhiMag_GPU222 <<< DimPhiMagGrid, DimPhiMagBlock >>> 
    (phiR_d, phiI_d, phiMag_d, numK);
}

void computeQ_GPU222(int numK, int numX,
                  ...
                  float* Qr_d, float* Qi_d)
{
  ...
  for (int QGrid = 0; QGrid < QGrids; QGrid++) {
    ...
    ComputeQ_GPU222 <<< DimQGrid, DimQBlock >>>
