...
int mycublasSgemm_fast(int transa, int transb, int m, int n, int k,
	const float *alpha, const float *A, int lda, const float *B, int ldb,
	const float *beta, float *C, int ldc)
{
	...
	matrixMulCUDA<16> << < grid, threads >> >(C, A, B, k, n, m, 1, 1 - transa, 1 - transb, *alpha, *beta);
...
void test_transpose(int rows, int cols){
    ...
    //float *X_dev = cuda_make_array(X_host, rows*cols);
    //int block_dim = BLOCK_DIM;
    //int grid_dim = (k*m + block_dim - 1) / block_dim;
    //transpose << <grid_dim, block_dim >> >(AT, A, k, m);
