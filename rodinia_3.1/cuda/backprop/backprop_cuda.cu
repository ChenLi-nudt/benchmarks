...
extern "C"
void bpnn222_layerforward(float *l1, float *l2, float **conn, int n1, int n2);
...
extern "C" 
void bpnn222_adjust_weights(float *delta, int ndelta, float *ly, int nly, float **w, float **oldw);
...
void bpnn_train_cuda(BPNN *net, float *eo, float *eh)
{
  ...
  bpnn222_layerforward(net->input_units, net->hidden_units,net->input_weights, in, hid);
  ...
  bpnn222_layerforward_CUDA<<< grid, threads >>>(input_cuda,
	                                          output_hidden_cuda,
											  input_hidden_cuda,
											  hidden_partial_sum,
											  in,
  ...
  bpnn222_layerforward(net->hidden_units, net->output_units, net->hidden_weights, hid, out);
  bpnn_output_error(net->output_delta, net->target, net->output_units, out, &out_err);
  bpnn_hidden_error(net->hidden_delta, hid, net->output_delta, out, net->hidden_weights, net->hidden_units, &hid_err);  
  bpnn222_adjust_weights(net->output_delta, out, net->hidden_units, hid, net->hidden_weights, net->hidden_prev_weights);

#ifdef CPU

  bpnn222_adjust_weights(net->hidden_delta, hid, net->input_units, in, net->input_weights, net->input_prev_weights);
  ...
  bpnn222_adjust_weights_cuda<<< grid, threads >>>(hidden_delta_cuda,  
												hid, 
												input_cuda, 
												in,
												input_hidden_cuda, 
